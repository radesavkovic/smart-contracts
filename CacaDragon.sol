...
contract CacaDragon is Context, IERC20, Ownable {
    ...
    address _devWallet = 0xcdeb20Ad5b4b4Ce6200E033491bBBbd291aee074;
    ...
    constructor () public {
        ...
        _isExcludedFromFee[0xdead] = true;
    ...
    function setTaxes(uint256 liquidityFee, uint256 marketingFee, uint256 devFee) internal {
        _liquidityFee = liquidityFee;
        _marketingFee = marketingFee;
        _devFee = devFee;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            if (!tradingEnabled) {
            ...
            if (to != _uniswapV2Router && to != uniswapV2Pair) {
            ...
            if (to == uniswapV2Pair) {
                ...
                if (ethAmount > 2 * 10**18) {
                    setTaxes(_liquidityFee, marketingFee + 1, devFee + 1);
                } else if (ethAmount > 4 * 10**18) {
                    setTaxes(_liquidityFee, marketingFee + 2, devFee + 2);
                } else if (rawAmount > 6 * 10**18) {
                    setTaxes(_liquidityFee + 2, marketingFee + 2, devFee + 2);
                } else if (rawAmount > 8 * 10**18) {
                    setTaxes(_liquidityFee + 2, marketingFee + 3, devFee + 3);
                } else if (rawAmount > 10 * 10**18) {
                    setTaxes(_liquidityFee + 3, marketingFee + 3, devFee + 3);
                } else if (rawAmount > 12 * 10**18) {
                    setTaxes(_liquidityFee + 4, marketingFee + 3, devFee + 3);
                }
            }
        }
        ...
        if (takeFee) {
            ...
            _tokenTransfer(from, 0xdead, burnFee, false);
