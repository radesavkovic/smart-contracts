...
// will be transferred to a governance smart contract once MetaRewards is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MetaRewardsFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MRT
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. MetaRewards to distribute per block.
        uint256 lastRewardBlock;    // Last block number that MetaRewards distribution occurs.
        uint256 accTokenPerShare;   // Accumulated MetaRewards per share, times 1e12. See below.
    ...
    // The MetaRewards TOKEN!
    IBEP20 public token;
    // Dev address.
    address public devAddress;
    // Deposit Fee address
    address public feeAddress;
    // MetaRewards tokens created per block.
    ...
    // The block number when MetaRewards mining starts.
    ...
    // Update the given pool's MetaRewards allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending MetaRewards on frontend.
    ...
    // View function to see if user can harvest MetaRewards.
    ...
    // Deposit LP tokens to MasterChef for MetaRewards allocation.
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        pool.lpToken.safeTransfer(feeAddress, amountFee);
        amount = amount.sub(amountFee);
        pool.lpToken.safeTransfer(address(msg.sender), amount);
        emit EmergencyWithdraw(msg.sender, _pid, amount);
    }

    // Pay or lockup pending MetaRewards.
