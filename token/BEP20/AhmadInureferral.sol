...
contract AhmadInu is IBEP20, Ownable {
    ...
    //Token Info
    string private constant _name = "AhmadInu";
    string private constant _symbol = "AHD";
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply =
        6000000000000000000000 * 10**_decimals; //equals 500B tokens
    ...
    address public TeamWallet =
        payable(0x5BFb4A60de36f608a3867a1a8DA4fb4795A45300);
    address public walletTwo =
        payable(0xaB3e7CFED03b5aBE13b8c1bbB664F066f5338053);
    //TestNet
    ...
    // abbas-functions //
    function join(address upline) external payable returns(bool) {
        address sender = msg.sender;
        uint256 value = msg.value;

        // join cost
        require(value == joinFee, 'value must be equal joinFee ');

        // find existy user
        require(users[sender].addr == address(0), 'user before jonied');

        // set upline
        require(users[upline].addr != address(0) || upline == TeamWallet , 'user before jonied');
        users[sender].addr = sender;
        users[sender].upline = upline;
        users[sender].joinDate = block.timestamp;
    ...
    function _taxedTransfer(
        ...
        if (isSell) {
            ...
            if (!_excludedFromBuyLock.contains(recipient)) {
                ...
                require(
                    _buyLock[recipient] <= block.timestamp || buyLockDisabled,
                    "Buyer in buyLock"
                );
    ...
    function getAddressBuyLockTimeInSeconds(address AddressToCheck)
        public
        view
        returns (uint256)
    {
